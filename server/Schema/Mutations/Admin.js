...
function GetSemester(group_name) {
    const date = new Date();
    let semester = Number(date.getFullYear().toString().charAt(3)) - Number(group_name.charAt(2)); // Год вступления, что бы узнать семестр для дисциплины
    if (semester < 0) {
...
export const SET_CLASSES = {
    ...
    async resolve(parent, { data, id_cathedra }) {
        ...
        for (const clas of classes) {
            ...
            let cathedra = null;
            for (const cath of cathedras) { // поиск кафедры
            ...
            for (const teacher of clas.teachers) {
                ...
                for (const teacherdb of teachers) {
                    if (teacherdb.dataValues.surname === surname
                ...
                if (!id_teacher) { // Если учитель не найден в бд
                    const new_teach = await db.teacher.create({ name, surname, patronymic, id_cathedra })
            ...
            for (const audience of clas.audiences) {
                ...
                for (const audiencedb of audiences) {
                    if (audiencedb.dataValues.name === audience) { // Найдена аудитория в базе
                        id_audience = audiencedb.dataValues.id;
                        arr_aud_ids.push(id_audience);
                        let checkCathedra = false;
                        for (const au of audiencedb.assigned_audiences) {// проверка на закрепленную кафедру
                            if (au.dataValues.id_cathedra === id_cathedra) {
                                checkCathedra = true;
                                break;
                            }
                        }
                        if (!checkCathedra) { // Если аудитория не закреплена за кафедрой
                            const new_assigned_audience = await db.assigned_audience.create({ id_audience, id_cathedra });
                            audiencedb.assigned_audiences.push(new_assigned_audience);
                        }
                        break;
                    }
                }
                if (!id_audience) { // Если аудитория не найдена в бд
                    let new_audience = await db.audience.create({ name: audience, id_type_class: 1, capacity: 0 });
                    id_audience = new_audience.dataValues.id;
                    arr_aud_ids.push(id_audience);
                    let new_assigned_audience = await db.assigned_audience.create({ id_audience, id_cathedra });
                    audiences.push({ ...new_audience, assigned_audiences: [new_assigned_audience] });
                }
            }
            // Поиск групп
            ...
            for (const group of clas.groups) {
                ...
                for (const groupdb of groups) {
                    if (groupdb.dataValues.name === group
                        && groupdb.specialty.cathedra.dataValues.short_name === clas.short_name_cathedra
                        && groupdb.specialty.dataValues.code === code_spec) {//найдена группа в базе
                        found_group = groupdb;
                        arr_groups_ids.push(found_group.dataValues.id);
                        break;
                    }
                }
                if (!found_group) {//Если не найдена группа
                    ...
                    for (const cath of cathedras) {
                        ...
                        if (String(cath.dataValues.short_name) === String(clas.short_name_cathedra)) {
                            ...
                            for (const spec of specialties) {
                                if (spec.dataValues.code === code_spec
                                    && spec.dataValues.id_cathedra === cath.dataValues.id) { // Найдена специальность с кафедрой
                    ...
                    if (found_cathedra) {// если кафедра найденa
                        if (!found_specialty) {//если не найдена специальность
                            found_specialty = await db.specialty.create({ name: "", code: code_spec, id_cathedra: found_cathedra.dataValues.id });
                            found_specialty.cathedra = found_cathedra;
                            specialties.push(found_specialty);
                        }
                    }
                    else {// если кафедра не найденa
                        found_cathedra = await db.cathedra.create({ name: "", short_name: clas.short_name_cathedra });
                    ...
                    found_group = await db.group.create({ name: group, number_students: 0, semester, id_specialty: found_specialty.dataValues.id });
                    arr_groups_ids.push(found_group.dataValues.id);
