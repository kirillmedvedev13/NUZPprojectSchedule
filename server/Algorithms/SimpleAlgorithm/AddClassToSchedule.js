...
function AddSchedule(
  ...
  // Проход по числителю
  // Поиск окон вниз
  for (let h = number_pair - 1; h >= 0; h--) {
    // Если есть предидущей пара то сразу выход
    if (h === number_pair - 1 && temp[day_week][h][1].clas.length || temp[day_week][h][3].clas.length) {
      break;
    }
    // Если нету предидущей пары снизу
    if ((pair_type === 1 || pair_type === 3) && !temp[day_week][h][1].clas.length && !temp[day_week][h][3].clas.length) {
      // Если окно
      if (number_pair - h > 1) {
        temp[day_week][h][1].isAvailable = false;
        temp[day_week][h][3].isAvailable = false;
      }
      else {
        temp[day_week][h][1].isAvailable = true;
        temp[day_week][h][3].isAvailable = true;
      }
    }
  }
  // Поиск окон вверх
  for (let h = number_pair + 1; h < max_pair; h++) {
    // Если есть следующая пара то сразу выход
    if (h === number_pair + 1 && temp[day_week][h][1].clas.length || temp[day_week][h][3].clas.length) {
      break;
    }
    // Если нету следующей пары вверх
    if ((pair_type === 1 || pair_type === 3) && !temp[day_week][h][1].clas.length && !temp[day_week][h][3].clas.length) {
      // Если окно
      if (h - number_pair > 1) {
        temp[day_week][h][1].isAvailable = false;
  ...
  // Проход по знаменателю
  // Поиск окон вниз
  for (let h = number_pair - 1; h >= 0; h--) {
    // Если есть предидущей пара то сразу выход
    if (h === number_pair - 1 && temp[day_week][h][2].clas.length || temp[day_week][h][3].clas.length) {
      break;
    }
    // Если нету предидущей пары снизу
    if ((pair_type === 2 || pair_type === 3) && !temp[day_week][h][2].clas.length && !temp[day_week][h][3].clas.length) {
      // Если окно
      if (number_pair - h > 1) {
