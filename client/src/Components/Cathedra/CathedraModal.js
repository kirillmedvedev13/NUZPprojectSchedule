        ...
        <Button variant="primary" onClick={(e) => {
            if (item.name) {
                mutateFunction(variables).then((res) => {
                    CreateNotification(item.id ? res.data.UpdateCathedra : res.data.CreateCathedra)
                    handleCloseModal();
                })
            }
            else {
            ...
            <>
                <Modal size="lg" show={isopen} onHide={this.handleClose}>
                    <Modal.Header closeButton>
                        <Modal.Title>{item.id ? "Редагувати запис кафедри" : "Створити запис кафедри"}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form noValidate validated={this.state.validated}>
                            <Form.Group as={Row} className="my-2 mx-2">
                                ...
                                <Col>
                                    <Form.Control required placeholder="Кафедра" value={item.name} onChange={(e) => {
